...
namespace CA.Engine
{
	public class Calc : INotifyPropertyChanged
	{
		...
		//private double step;
		private bool[,] sm = {{true, true, true}, {true, false, true}, {true, true, true}};
		//private int[] liv = {1, 3}; // {0,1,4..8}        {Conway: 3}
		//private int[] dth = {0, 2, 4, 5, 6, 7, 8}; // {0..2,4..8} {Conway: 0,1,4..8}
		private int[] liv = {1, 2, 4, 6, 7, 8};
		private int[] dth = {};
		private bool[,] rcb = {{true, true, true},
								{true, true, true},
								{true, true, true}};
		...
		private void initializeSenescence() {
		...
		public void proceed() {
			...
			for(int  a = 0; a < (w); a++) {
				for(int b = 0; b < (h); b++) {
					...
					if(sm[1,0] && rcb[1,0]) { if(idCopy[a + w * (b-1)] != cd) { k++; } }
					if(sm[2,0] && rcb[2,0]) { if(idCopy[(a+1) + w * (b-1)] != cd) { k++; } }
					if(sm[0,1] && rcb[0,1]) { if(idCopy[(a-1) + w * b] != cd) { k++; } }
					if(sm[2,1] && rcb[2,1]) { if(idCopy[(a+1) + w * b] != cd) { k++; } }
					if(sm[0,2] && rcb[0,2]) { if(idCopy[(a-1) + w * (b+1)] != cd) { k++; } }
					if(sm[1,2] && rcb[1,2]) { if(idCopy[a + w * (b+1)] != cd) { k++; } }
